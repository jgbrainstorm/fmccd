...
for k in ext:
    ...
    for i in range(Nfits):
        x0=pf.getdata(CatFits[15]).field('x_image')
        y0=pf.getdata(CatFits[15]).field('y_image')
        flux=pf.getdata(CatFits[15]).field('flux_best')
    ...
    pl.title('y tilt: -100')
    pl.ylim(-30,30)
    pl.savefig('hexmove_ytilt_100_ext_'+str(k)+'.png')
    pl.close()
