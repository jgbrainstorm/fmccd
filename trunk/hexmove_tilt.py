...
for k in ext:
    ...
    pl.ylim(-30,30)
    pl.savefig('hexmove_xtilt100_ext_zenith_'+str(k)+'.png')
    pl.close()
os.system('convert -delay 200 -loop 0 hexmove_xtilt100_ext_zenith*.png '+'hexmove_xtilt100_ext_zenith.gif')  
...
for k in ext:
    ...
    Nfits=len(CatFits)
    for i in range(Nfits):
        x0=pf.getdata(CatFits[10]).field('x_image')
        y0=pf.getdata(CatFits[10]).field('y_image')
        flux=pf.getdata(CatFits[10]).field('flux_best')
        ok=flux>np.mean(flux)
        x0=x0[ok]
        y0=y0[ok]
    Mxdiff=[]
    ...
    pl.errorbar(xshift,Mydiff,yerr=SDydiff,fmt='bo')
    pl.plot(xshift,by*xshift+aay,'r-')
    pl.xlabel('Hexpod shift in y direction (micron)')
    pl.ylabel('image shift in y direction (pixels)')
    pl.text(0.2,0.85,'slope: '+str(round(by,3))+r'$\pm$'+str(round(SEby,3)),transform = ax.transAxes)
    pl.text(0.2,0.8,'Itcpt: '+str(round(aay,3))+r'$\pm$'+str(round(SEay,3)),transform = ax.transAxes)
    pl.title('Extension: '+str(k))
    ...
    pl.text(0.2,0.85,'slope: '+str(round(bx,3))+r'$\pm$'+str(round(SEbx,3)),transform = ax.transAxes)
    pl.text(0.2,0.8,'Itcpt: '+str(round(aax,3))+r'$\pm$'+str(round(SEax,3)),transform = ax.transAxes)
    pl.title('y tilt: 100'+' at zenith')
    pl.ylim(-30,30)
    pl.savefig('hexmove_ytilt100_ext_zenith_'+str(k)+'.png')
    pl.close()
os.system('convert -delay 200 -loop 0 hexmove_ytilt100_ext_zenith*.png '+'hexmove_ytilt100_ext_zenith.gif')  
